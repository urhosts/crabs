<template>
  <BasicModal v-bind="$attrs" @register="registerModal" destroyOnClose :title="title" :width="896" @ok="handleSubmit">
      ...
      <div style="width: 100%;text-align: center" >
          <a-button @click="handleRecognition" pre-icon="ant-design:check" type="primary">识 别</a-button>
      </div>
  </BasicModal>
</template>
...
<script lang="ts" setup>
    ...
    import {formSchema} from '../RemainingBait.data';
    import {saveOrUpdate,recognition} from '../RemainingBait.api';
    // Emits声明
    ...
    async function handleRecognition(v) {
      try {
          //  //提交表单
          let values = await validate();
          ...
          setModalProps({confirmLoading: true});
          await recognition(values, isUpdate.value).then(function (data) {
            console.log(data);
            setFieldsValue({
